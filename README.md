...
[![Encryption: Vault Transit](https://img.shields.io/badge/Crypto-Vault%20Envelope%20AES--256--GCM-orange.svg)](#architecture)
...
- **Transparent Vulnerability Remediation**: Features a genuine deliberate vulnerability exercise (JWT algorithm confusion / lg: none bypass), a working exploit script, before/after captures, and remediation diff in [vuln-writeup.md](docs/vuln-writeup.md).
...
Evaluated with scripts/eval/run_eval.py across 300 benign baseline training sessions, 100 held-out benign test sessions, 150 attack sessions across 3 distinct MITRE identity techniques, and 30 held-out blended anomaly sessions:

| Identity Attack Vector | MITRE ATT&CK | Test Samples | Precision | Recall | F1-Score | Detection Status |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Credential Stuffing** | T1110.003 | 50 | **0.8929** | **1.0000** | **0.9434** | Verified Caught |
| **Impossible Travel** | T1078 | 50 | **0.8929** | **1.0000** | **0.9434** | Verified Caught |
| **MFA Push-Bombing** | T1621 | 50 | **0.8929** | **1.0000** | **0.9434** | Verified Caught |
| **Held-out Blended Pattern** | T1078/T1110 | 30 | **0.8333** | **1.0000** | **0.9091** | Generalisation Verified |

- **Benign False Positive Rate (FPR)**: **6.00%** (6/100 held-out benign sessions).
- **Latency Overhead**: **+2.098 ms** per request for the complete cryptographic stack (JWT verify + AES-256-GCM envelope DEK wrap/unwrap + field-level PII encryption + audit log write).
